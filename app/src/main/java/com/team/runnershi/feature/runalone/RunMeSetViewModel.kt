...
class RunMeSetViewModel : ViewModel() {
...
//        ldIsKmPassed.value = (totalMeter / 1000 >= distStandard)
    ...
    fun calPace(curLocation: Location) {
        if (prevLocation == null) {
            ...
            if (paceMinute >= 60) {
                ldPace.postValue("-'--\"")
                "Pace PostValue: -'--\"".logDebug(this@RunMeSetViewModel)
            } else {
    ...
    fun showRunLeftTime(prevLeftTime: Int) {
        ...
        "initial (prevLeftTime $prevLeftTime) (prevProgress: $prevProgress)".logDebug(this@RunMeSetViewModel)
        timer.scheduleAtFixedRate(object : TimerTask() {
            override fun run() {
                leftTime += 1
                val newLeftTime = this@RunMeSetViewModel.runTime - leftTime
